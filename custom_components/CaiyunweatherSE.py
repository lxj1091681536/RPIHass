...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    download_path = config.get(CONF_DOWNLOAD_DIR)
        ...
        def do_download():
            ...
            try:
                ...
                if req.status_code == 200:
                    ...
                    _Log.info("%s -> %s", url, final_path)

                    with open(final_path, 'wb') as fil:
                        for chunk in req.iter_content(1024):
                            fil.write(chunk)

                    _Log.info("Downloading of %s done", url)

            except requests.exceptions.ConnectionError:
                _Log.exception("ConnectionError occured for %s", url)
    ...
    hass.services.async_register(DOMAIN, SERVICE_DOWNLOAD_FILE, download_file,
    ...
    if  CONF_REALTIME in monitored_conditions:
        ...
        if isinstance(realtimeSensor, list):
            if len(realtimeSensor) == 0:
                ...
                dev.append(CaiyunSensor(CONF_REALTIME, 'temperature', sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
            for sensor in realtimeSensor:
                ...
                dev.append(CaiyunSensor(CONF_REALTIME, sensor, sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
    if  CONF_PRECIPITATION in monitored_conditions:
        ...
        if isinstance(precipitationSensor, list):
            if len(precipitationSensor) == 0:
                ...
                dev.append(CaiyunSensor(CONF_PRECIPITATION, 'nearest_precipitation_distance', sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
            for sensor in precipitationSensor:
                ...
                dev.append(CaiyunSensor(CONF_PRECIPITATION, sensor, sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
    ...
    if  CONF_HOURLY in monitored_conditions:
        ...
        if isinstance(hourlySensor, list):
            if len(hourlySensor) == 0:
                ...
                dev.append(CaiyunSensor(CONF_HOURLY, 'description', sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
            for sensor in hourlySensor:
                sensor_name = HOURLY_TYPE[sensor][0]
                measurement = HOURLY_TYPE[sensor][1]
                icon =  HOURLY_TYPE[sensor][2]
                dev.append(CaiyunSensor(CONF_HOURLY, sensor, sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
    if  CONF_MINUTELY in monitored_conditions:
        ...
        if isinstance(minutelySensor, list):
            if len(minutelySensor) == 0:
                ...
                dev.append(CaiyunSensor(CONF_MINUTELY, 'description', sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
            for sensor in minutelySensor:
                ...
                dev.append(CaiyunSensor(CONF_MINUTELY, sensor, sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
    ...
    if  CONF_DAILY in monitored_conditions:
        ...
        if isinstance(dailySensor, list):
            if len(dailySensor) == 0:
                ...
                dev.append(CaiyunSensor(CONF_DAILY, 'coldRisk', sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
            for sensor in dailySensor:
                sensor_name = DAILY_TYPE[sensor][0]
                measurement = DAILY_TYPE[sensor][1]
                icon = DAILY_TYPE[sensor][2]
                dev.append(CaiyunSensor(CONF_DAILY, sensor, sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
    if CONF_ALARM in monitored_conditions:
        ...
        if isinstance(alarmSensor,list):
            for sensor in alarmSensor:
                ...
                dev.append(CaiyunSensor(CONF_ALARM,  sensor, sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
        ...
        if isinstance(daySensor, list):
            for day in daySensor:
                for sensor in dailySensor:
                    ...
                    measurement = DAILY_TYPE[sensor][1]
                    icon = DAILY_TYPE[sensor][2]
                    dev.append(CaiyunSensor(DAY_TYPE[day][0], sensor, sensor_name,measurement,icon,api_key,None, hass, download_file, download_path))
    dev.append(CaiyunSensor('realtime_update', 'realtime_update', 'realtime_update_time',None,'mdi:clock',api_key, None, hass, download_file, download_path))
    dev.append(CaiyunSensor('forecast_update', 'forecast_update', 'forecast_update_time',None,'mdi:clock',api_key, None, hass, download_file, download_path))
    add_devices(dev,True)
    entity_id = config.get(CONF_DEVICES)
    dev0 = []
    for i in range(len(entity_id)):
        dev0.append(CaiyunSensor(CONF_MINUTELY, 'tracker', entity_id[i]+'_description', None, 'mdi:heart-half-full', api_key, entity_id[i], hass, download_file, download_path))
...
class CaiyunSensor(Entity):
    ...
    def __init__(self,sensor_Type,sensor,sensor_name,measurement,icon,api_key, entity_id, hass, download_file, download_path):
        ...
        def creat_downdata_service(self,service_name, data_type, file_name,  api_key, longitude, latitude,download_path):
            ...
            self.hass.services.register(DOMAIN, service_name, self.download_file,
                                   ...
                                   vol.Optional(ATTR_FILENAME,default= file_name):cv.string,
                                   vol.Optional(ATTR_SUBDIR): cv.string,}))
        ...
        if self._sensor ==  'forecast_update':
        ...
        if self._sensor ==  'realtime_update':
            self.forecast_path = creat_downdata_service(self,'Download_Caiyun_weatherData_forecast', 'forecast', 'forecast.json', self.api_key, self.longitude, self.latitude,self.download_path)
        if self._sensor ==  'forecast_update':
            self.realtime_path = creat_downdata_service(self,'Download_Caiyun_weatherData_realtime', 'realtime', 'realtime.json', self.api_key, self.longitude, self.latitude,self.download_path)
