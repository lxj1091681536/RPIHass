...
from homeassistant.const import (
    ...
    CONF_NAME, CONF_HOST, CONF_MAC, CONF_TIMEOUT, CONF_FRIENDLY_NAME)
...
# _LOGGER.error(os.path.realpath(__file__))
...
DEFAULT_MAX_TMEP = 30
DEFAULT_STEP = 1
DEFAULT_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)),'climate.db')
...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    hum_entity_id = config.get(CONF_HUM_SENSOR)
    ...
    async_add_devices([BLClimate(
            hass, db, name, target_temp, None, None, None, 'Auto', 50,
            None, 'off', False, DEFAULT_MAX_TMEP, DEFAULT_MIN_TMEP, 
            broadlink_device, temp_entity_id, hum_entity_id)])


class BLClimate(ClimateDevice):
    """Representation of a demo climate device."""

    def __init__(self, hass, db, name, target_temperature, target_humidity,
        ...
        self._fan_list = ['On Low', 'On Midium','On High', 'Auto']
        self._operation_list = ['heat', 'dry', 'cool', 'auto', 'off']
        self._swing_list = ['toggle']
        ...
        async_track_state_change(
            hass, hum_entity_id, self._async_hum_changed)
        
        hum_state = hass.states.get(hum_entity_id)
        if hum_state:
            self._async_update_hum(hum_state)
    ...
    def _async_update_hum(self, state):
        ...
        try:
            self._current_humidity = state.state
        except ValueError as ex:
            _LOGGER.error('Unable to update from sensor: %s', ex)

    @asyncio.coroutine
    def _async_hum_changed(self, entity_id, old_state, new_state):
        """Handle temperature changes."""
        if new_state is None:
            return
    ...
    def _cal_code(self):
        mode = {'heat':400, 'cool':100, 'auto':0, 'off':0 ,'dry':200 , 'idle':0}
        fan = {'On Low':1000, 'On Midium':2000,'On High':3000, 'Auto':0}
        code = fan[self._current_fan_mode] + mode[self._current_operation] + self._target_temperature
        if (code/100)%10 == 0:
            code = 0
        self._sendpacket(code)

    def _sendpacket(self,code,retry=2):
        """Send packet to device."""
        sendir = None
        ir = self._db.execute("SELECT code from MEDIA_NORMAL WHERE id = %d " % (code))
        for row in ir:
            sendir = b64decode(row[0]) 
        try:
